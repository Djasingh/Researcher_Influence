...
if __name__ == '__main__':
    ...
    scaled_op = True  #scale output if True

    train_ip_name = f"dataset/train_input_wo_null_thesis_{train_year}.npy"
    train_op_name = f"dataset/train_output_wo_null_thesis_{train_year}.npy"
    ...
    print(f"train op: {train_output[0]}")
    print(f"train output size:{train_output.shape}")
    
    test_ip_name = f"dataset/test_input_wo_null_thesis_{test_year}.npy"
    test_op_name = f"dataset/test_output_wo_null_thesis_{test_year}.npy"
    ...
    name = f"./exp_lc/path_model_{batch_size}_{latent_dim}_{lr}_{epochs}_{train_year}_{test_year}_{error}_{scaled_op}_{dropout}.png"
    ...
    if save :
        ...
        print(f"model saved")
    with open('./exp_results/method3_result_wo_null_thesis_512.csv', 'a') as csvfile:
        writer = csv.writer(csvfile, delimiter=",")
        writer.writerow([max_num_seq, max_seq_len, embedding_size, batch_size, latent_dim, lr, epochs, train_year, test_year,
