...
def mod_get_paths(edge_list, root):
    '''
    return list of paths from root node to every nodes/list of list(path)
    '''
    G1 = nx.DiGraph()
    G1.add_edges_from(edge_list)
    all_nodes = [node for node in G1 if node != root]
    paths = [nx.shortest_path(G1, root, node) for node in all_nodes]
    return paths
