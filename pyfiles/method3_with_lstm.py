...
if __name__ == '__main__':
    ...
    train_ip_name = f"./dataset/train_input_m3_tsne_{train_year}.npy"
    train_op_name = f"./dataset/train_output_m3_tsne_{train_year}.npy"
    ...
    test_ip_name = f"./dataset/test_input_m3_tsne_{test_year}.npy"
    test_op_name = f"./dataset/test_output_m3_tsne_{test_year}.npy"
    ...
    activation = 'tanh'
    latent_dim = 32 #64 with tsne embedding
    batch_size = 32
    epochs = 25
    lr = 0.0001
    dropout = 0.01
    ...
    name = f"./exp_lc/path_model_{batch_size}_{latent_dim}_{lr}_{epochs}_{train_year}_{test_year}_{error}_{not_scaled_op}_{dropout}_all.png"
    learning_curves(history, name)
    print(f"learning curves created in dir : {name}")

    if save :
        config1 = f"{batch_size}_{latent_dim}_{lr}_{epochs}_{train_year}_{test_year}_{val_error}_{error}_{not_scaled_op}_{dropout}_all"
        model_save(model1, *enc_dec, config1)
        print(f"model saved")
    with open('./exp_results/method3_all_relation_kept.csv', 'a') as csvfile:
