...
class AGG_LAYER(Layer):
    def __init__(self, units, input_dim, latent_dim):
        self.units = units
    ...
    def backend_reshape(self, x, shape=(-1, 52, 512)):
        return  tf.keras.backend.reshape(x, shape)

    def call(self, inputs):
        return self.backend_reshape(tf.matmul(self.w, self.backend_reshape(inputs,(-1, self.units, self.latent_dim))), (-1, self.latent_dim))
...
def get_model(latent_dim, max_num_seq, max_seq_len, embedding_size, dense_size, lr, dropout):
    ...
    encoder_outputs, state_h, state_c = encoder(mod_en_in, training = True)
    agg_layer_h = AGG_LAYER(units=max_num_seq, input_dim=1, latent_dim=latent_dim)
    agg_layer_c = AGG_LAYER(units=max_num_seq, input_dim=1, latent_dim=latent_dim)
...
def model_save(model, encoder, decoder, config1):
    model.save('exp_models/path_'+config1)
    encoder.save('exp_models/path_enc_'+config1)
    decoder.save('exp_models/path_dec_'+config1)
    return
...
if __name__ == '__main__':
    ...
    train_ip_name = f"./dataset/train_input_m3_tsne_ns_tree_oldrk_{train_year}.npy" #train_input_m3_tsne_ns_tree_oldrk_1950_1980.npy
    train_op_name = f"./dataset/train_output_m3_tsne_ns_tree_oldnrk_{train_year}.npy"
    ...
    test_ip_name = f"./dataset/test_input_m3_tsne_ns_tree_oldrk_{test_year}.npy"
    test_op_name = f"./dataset/test_output_m3_tsne_ns_tree_oldrk_{test_year}.npy"
    ...
    activation = 'tanh'
    latent_dim = 64 #64 with tsne embedding
    batch_size = 32
    epochs = 25
    lr = 0.0001
    dropout = 0.01
    ...
    print(f"activation : {activation},latent dim : {latent_dim}, Batch size : {batch_size}, lr : {lr}, epochs : {epochs}, dropout : {dropout}, scaled : {not_scaled_op}")
    
    model, other = get_model(latent_dim, max_num_seq, max_seq_len, embedding_size, dense_size, lr, dropout)
    print("Model created")
    ...
    name = f"./exp_lc/path_model_{batch_size}_{latent_dim}_{lr}_{epochs}_{train_year}_{test_year}_{error}_{not_scaled_op}_{dropout}_oldrk.png"
    learning_curves(history, name)
    print(f"learning curves created in dir : {name}")

    if save :
        config1 = f"{batch_size}_{latent_dim}_{lr}_{epochs}_{train_year}_{test_year}_{val_error}_{error}_{not_scaled_op}_{dropout}_oldrk"
        model_save(model1, *enc_dec, config1)
        print(f"model saved")
    with open('./exp_results/method3_old_relation_kept.csv', 'a') as csvfile:
        writer = csv.writer(csvfile, delimiter=",")
        writer.writerow([activation,"LSTM", max_num_seq, max_seq_len, embedding_size, batch_size, latent_dim, lr, epochs, train_year, test_year,
                         val_error, error, not_scaled_op, dropout])
